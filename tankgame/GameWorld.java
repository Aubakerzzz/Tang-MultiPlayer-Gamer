...
public final class GameWorld extends JPanel implements Runnable, Observer {
    ...
    private void loadSprites(){    	
        ...
        sprites.put("back", getSprite("Resources/title.png"));
    ...
    public void drawFrame(int w, int h, Graphics2D g2) {
        ...
        ListIterator<PlayerShip> playersIterator = getPlayers();
        while (playersIterator.hasNext()) {
            PlayerShip player = playersIterator.next();

            if (player.getScore() >= 200) {
                gameOver = true;
                break; // No need to continue checking if one tank has reached the score
            }

            // Rest of the player update logic
        }

        // If any tank's score is >= 200, display "Game Over!" and stop the game
        if (gameOver) {
            // Display "Game Over!" message
            g2.setColor(Color.RED);
            g2.setFont(new Font("Arial", Font.BOLD, 200));
            String gameOverMessage = "Game Over!";
            int messageWidth = g2.getFontMetrics().stringWidth(gameOverMessage);
            int x = (size.x - messageWidth) / 2;
            int y = size.y / 2;
            g2.drawString(gameOverMessage, x, y);
            addBackground(new Background(size.x,size.y,speed, sprites.get("back")));

            // Stop game logic (if needed)
            // For example: thread = null; to stop the game loop

            return; // Exit the drawFrame method
        }
    ...
    public void run() {
        ...
        while (thread == me) {
            ...
            repaint();
                ...
                GameSounds.play("Resources/background.wav");
